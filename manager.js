...
function showView(viewId) {
    // Retirer la classe active de toutes les vues et boutons
    document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
    document.querySelectorAll('.nav-btn').forEach(b => b.classList.remove('active'));
    
    // Ajouter la classe active à la vue sélectionnée
    ...
    if (titleElement) {
        titleElement.innerText = titles[viewId] || 'KES Manager';
    }
    
    // Actions spécifiques selon la vue
    if (viewId === 'techniciens') {
        renderTechniciens();
    }
    if (viewId === 'stats') {
        renderStats();
